...
export async function fetchExcludedDbIds(excludeDbId: string): Promise<Set<string>> {
  ...
  try {
    ...
    for (const row of resp.results || []) {
      ...
      if (!active) continue;
      const raw =
        row.properties?.["DB ID"]?.rich_text?.[0]?.plain_text?.trim() || "";
      const id = raw.replace(/-/g, "");
      if (id.length === 32) excluded.add(id);
    }
  } catch (e) {
    console.error("Failed to load exclude list:", e);
  }
  return excluded;
}
...
export async function fetchIconMap(iconMapDbId: string): Promise<Map<string, string>> {
  ...
  try {
    ...
    for (const row of resp.results || []) {
      const dbId =
        row.properties?.["DB ID"]?.rich_text?.[0]?.plain_text?.trim().replace(/-/g, "") || "";
      const icon =
        row.properties?.["아이콘"]?.rich_text?.[0]?.plain_text?.trim() || "";
  ...
  await Promise.allSettled(
    Array.from(dbMap.entries()).map(async ([dbId, page]) => {
      try {
        ...
        const action = itemTitle
          ? `${itemTitle} ${isNew ? "작성" : "수정"}`
          : "내용 업데이트";
        const dbName = extractDbTitle(db);
        results.push({
          entityId: dbId,
          entityType: "db",
          title: `${dbName} - ${action}`,
...
export async function fetchSnapshotUpdates(
  ...
  const settled = await Promise.allSettled(
    pageIds.map(async (page_id) => {
      ...
      const itemTitle = extractItemTitle(page);
      const isNew = page.created_time === page.last_edited_time;
      const action = isNew ? "페이지 추가" : "페이지 수정";
      const pageId = (page.id || "").replace(/-/g, "");
      return {
        entityId: pageId,
        entityType: "page" as const,
        title: `${itemTitle || "(페이지)"} - ${action}`,
        url: page.url,
  ...
  return settled
    .filter(
      (r): r is PromiseFulfilledResult<UpdateItem> => r.status === "fulfilled"
    )
    .map((r) => r.value);
}